...
- No-persona Codex installs default to the `codex` MCP surface, which keeps
  exact `memory_search` and `memory_query` available.
...
- The `codex` MCP surface exposes the normal memory belt, transcript recall,
  exact `memory_search`, structured `memory_query`, `memory_stats`, and
  `memory_whereami`, while hiding operator/admin tools.
- `memory_diagnose(mode="whereami")` reports project identity/root provenance
  alongside persona and transcript runtime fields.
...
- Explicit `CHIMERA_MEMORY_PROJECT_ID` pairs correctly with a single explicit
  `CHIMERA_MEMORY_PROJECT_ROOT`, even when the root folder's derived id differs.
...
- MCP stdio startup smoke passed with temp home/config, no `TRANSCRIPT_PERSONA`,
  ...
  `memory_diagnose(mode="whereami")` resolved project id/root.
- Live Codex Desktop config was updated after the temp gate passed:
  `C:\Users\charl\.codex\config.toml` now uses project profile
  `Chimera-Memory`, project root `C:\Github\Chimera-Memory\.chimera-memory`,
  `CHIMERA_MEMORY_MCP_SURFACE=codex`, and `python -m chimera_memory.cli serve`.
  Backup: `C:\Users\charl\.codex\config.toml.bak-cm-20260525T150730Z`.
...
- `start_memory_watcher` appears to schedule only the last resolved project
  root. That needs a focused test and fix before relying on multi-project live
  project-memory watching.
...
2. Fix project watcher coverage:
   add a regression test for multiple project roots, then update
   `start_memory_watcher` so every configured project root is scheduled.
